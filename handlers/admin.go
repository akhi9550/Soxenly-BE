...
// @Summary		Delete User
// @Description	using this handler admins can delete an user
// @Tags			Admin User Management
// @Accept			json
// @Produce		json
// @Security		Bearer
...
func DeleteUser(c *gin.Context) {
	...
	if err != nil {
		errs := response.ClientResponse(http.StatusInternalServerError, "user could not be deleted", nil, err.Error())
		c.JSON(http.StatusInternalServerError, errs)
		return
	}
	success := response.ClientResponse(http.StatusOK, "Successfully deleted the user", nil, nil)
	c.JSON(http.StatusOK, success)
}
...
func DeleteProductImage(c *gin.Context) {
	...
	url := c.Query("url")
	err := usecase.DeleteImage(productID, url)
	if err != nil {
		errs := response.ClientResponse(http.StatusInternalServerError, "image could not be deleted", nil, err.Error())
		c.JSON(http.StatusInternalServerError, errs)
		return
	}
