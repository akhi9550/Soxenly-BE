...
func UpdateCategory(c *gin.Context) {
	...
	ok, err := usecase.UpdateCategory(category)
	if err != nil {
		errs := response.ClientResponse(http.StatusInternalServerError, "could not update the category", nil, err.Error())
...
func UploadCategoryImage(c *gin.Context) {
	...
	url, err := usecase.UpdateCategoryImage(id, file)
	if err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "Could not upload image", nil, err.Error())
		c.JSON(http.StatusBadRequest, errorRes)
		return
	}

	successRes := response.ClientResponse(http.StatusOK, "Successfully uploaded category image", url, nil)
