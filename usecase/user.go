...
func GoogleLoginOrSignup(email, firstname, lastname string) (*models.TokenUser, error) {
	userEmail, err := repository.CheckUserExistsByEmail(email)
	if err != nil {
		return &models.TokenUser{}, errors.New("error with server")
	}
	...
	if userEmail == nil {
		...
		userData, err := repository.UserSignUp(newUser)
		if err != nil {
			return &models.TokenUser{}, errors.New("could not create user via Google")
		}
...
func UpdateQuantityAdd(id, productID int, size string) error {
	productExist, err := repository.ProductExistCart(id, productID, size)
	if err != nil {
		return err
	}
	if !productExist {
	...
	stock, err := repository.ProductStock(productID, size)
	if err != nil {
		return err
	}

	stockfromcart, err := repository.StockFormCart(id, productID, size)
	if err != nil {
		return err
	}

	if stockfromcart >= stock {
		return errors.New("its maximum no more updation")
	}

	err = repository.UpdateQuantityAdd(id, productID, size)
	if err != nil {
		return err
	}
	...
	FinalPrice = FinalPrice * float64(stockfromcart+1)
	err = repository.UpdateTotalPrice(id, productID, FinalPrice, size)
	if err != nil {
		return err
	}
	return nil
...
func UpdateQuantityless(id, productID int, size string) error {
	productExist, err := repository.ProductExistCart(id, productID, size)
	if !productExist {
	...
	stock, err := repository.ExistStock(id, productID, size)
	if err != nil {
		return err
	}
	if stock <= 1 {
	...
	err = repository.UpdateQuantityless(id, productID, size)
	if err != nil {
		return err
	}
	stockfromcart, err := repository.StockFormCart(id, productID, size)
