...
func UserRoutes(r *gin.RouterGroup, db *gorm.DB) *gin.RouterGroup {
	...
	r.POST("/google-login", handlers.GoogleLogin)
	r.GET("/category", handlers.GetCategory)
	r.GET("/banner", handlers.GetBannersUser)
	...
	products := r.Group("/products")
	{
		...
		products.GET("/details", handlers.GetProductDetails)
		products.GET("/new", handlers.GetNewArrivals)
	}
	...
	r.Use(middleware.UserAuthMiddleware())
	{
		...
		checkout := r.Group("/checkout")
		{
			...
			checkout.GET("/print", handlers.PrintInvoice)
