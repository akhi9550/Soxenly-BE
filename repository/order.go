...
func GetOrderDetails(userId int, page int, count int) ([]models.FullOrderDetails, error) {
	...
	err := db.DB.Raw("SELECT id as order_id, final_price, shipment_status, payment_status FROM orders WHERE user_id = ? ORDER BY id DESC LIMIT ? OFFSET ?", userId, count, offset).Scan(&orderDetails).Error
	if err != nil {
		return nil, err
	}
	...
	for _, od := range orderDetails {
		...
		err := db.DB.Raw(`SELECT product_id, products.name as product_name, quantity, order_items.total_price, 
		                 (SELECT url FROM images WHERE images.product_id = products.id LIMIT 1) as image_url 
		                 FROM order_items INNER JOIN products ON order_items.product_id = products.id 
		                 WHERE order_id = ?`, od.OrderId).Scan(&orderProductDetails).Error
		if err != nil {
			return nil, err
		}
...
func ApproveOrder(orderID int) error {
	err := db.DB.Exec("UPDATE orders SET shipment_status = 'confirmed' , approval = 'true', updated_at = NOW() WHERE id = ?", orderID).Error
	if err != nil {
		return err
	}
	return nil
...
func ShipOrder(orderID int) error {
	err := db.DB.Exec("UPDATE orders SET shipment_status = 'shipped', updated_at = NOW() WHERE id = ?", orderID).Error
	if err != nil {
		return err
	}
	return nil
...
func CancelOrders(orderID int) error {
	status := "cancelled"
	err := db.DB.Exec("UPDATE orders SET shipment_status = ? , approval='false', updated_at = NOW() WHERE id = ? ", status, orderID).Error
	if err != nil {
		return err
	}
...
func UpdateQuantityOfProduct(orderProducts []models.OrderProducts) error {
	for _, od := range orderProducts {
		if err := db.DB.Exec("UPDATE product_variants SET stock = stock + ? WHERE product_id = ? AND size = ?", od.Stock, od.ProductId, od.Size).Error; err != nil {
			return err
		}
	}
	return nil
...
func OrderItems(ob models.OrderIncoming, price float64) (int, error) {
	var id int
	query := `
    INSERT INTO orders (created_at , updated_at, user_id , address_id , payment_method_id , final_price)
    VALUES (NOW(), NOW(), ?, ?, ?, ?)
    RETURNING id`
...
func UpdateOrder(orderID int) error {
	err := db.DB.Exec("UPDATE orders SET Shipment_status = 'order placed', updated_at = NOW() WHERE id = ?", orderID).Error
	if err != nil {
		return err
	}
	return nil
...
func UpdateCartAfterOrder(userID, productID int, quantity float64) error {
	...
	err = db.DB.Exec("UPDATE product_variants SET stock = stock - ? WHERE product_id = ? AND id = (SELECT id FROM product_variants WHERE product_id = ? LIMIT 1)", quantity, productID, productID).Error
	if err != nil {
		return err
	}
...
func UpdateStockOfProduct(orderProducts []models.OrderProducts) error {
	for _, ok := range orderProducts {
		if err := db.DB.Exec("UPDATE product_variants SET stock = stock + ? WHERE product_id = ? AND size = ?", ok.Stock, ok.ProductId, ok.Size).Error; err != nil {
			return err
		}
	}
	return nil
...
func GetAllOrderDetailsBrief(page, count int) ([]models.CombinedOrderDetails, error) {
	...
	err := db.DB.Raw(`
		SELECT 
			...
			addresses.state,
			addresses.pin,
			STRING_AGG(COALESCE(order_items.size, 'N/A'), ', ') as size,
		...
		LIMIT ? OFFSET ?`, count, offset).Scan(&orderDatails).Error
	if err != nil {
		return []models.CombinedOrderDetails{}, nil
	}
...
func DecreaseStock(productID int, size string, quantity int) error {
	err := db.DB.Exec("UPDATE product_variants SET stock = stock - ? WHERE product_id = ? AND size = ?", quantity, productID, size).Error
	if err != nil {
		return err
	}
	return nil
...
func ProductStockMinus(productID, stock int) error {
	err := db.DB.Exec("UPDATE product_variants SET stock = stock - ? WHERE product_id = ? AND id = (SELECT id FROM product_variants WHERE product_id = ? LIMIT 1)", stock, productID, productID).Error
	if err != nil {
		return err
	}
	return nil
...
func FindStock(id int) (int, error) {
	var stock int
	err := db.DB.Raw("SELECT SUM(stock) FROM product_variants WHERE product_id = ?", id).Scan(&stock).Error
	if err != nil {
		return 0, err
	}
